...
NSArray<Class> *as_getMatchedClasses(BOOL(^predicate)(Class cls)) {
...
@implementation NSObject (ASEventRouter)
+ (id)as_objectWithBuffer:(void *)valueLoc type:(const char *)argType {
...
- (void)as_getValue:(void *)valueLoc type:(const char *)argType {
...
- (id)as_performSelector:(SEL)aSelector withObjects:(NSArray *)objects {
    ...
    for (NSInteger i = 0; i < objects.count; i ++) {
        [invocation as_setArgumentObject:objects[i] atIndex:i+2];
    }
    ...
    if (!signature.methodReturnLength) {
        ...
        return [NSObject as_objectWithBuffer:valueLoc type:signature.methodReturnType];
    }
}
...
@implementation NSInvocation (ASEventRouter)
